 ...
 */
export function mapStepToAction(step: TestStep): MappedAction {
      ...
      return {
        execute: async (page: any) => {
            ...
            if (assertionType === 'toBeVisible') {
                ...
                if (!text.includes(assertionValue)) throw new Error(`Expected text "${assertionValue}", found "${text}"`);
            } else if (assertionType === 'toHaveValue') {
                const val = await loc.inputValue();
                if (val !== assertionValue) throw new Error(`Expected value "${assertionValue}", found "${val}"`);
            } else if (assertionType === 'toContainText') {
                const text = await loc.textContent();
                if (!text?.includes(assertionValue)) throw new Error(`Expected text to contain "${assertionValue}", found "${text}"`);
                ...
                if (val && actual !== val) throw new Error(`Expected attribute ${attr}="${val}", found "${actual}"`);
                if (!val && actual === null) throw new Error(`Expected attribute ${attr} to exist`);
            } else if (assertionType === 'toHaveCount') {
                const count = await loc.count();
                const expected = parseInt(assertionValue, 10);
                if (count !== expected) throw new Error(`Expected count ${expected}, found ${count}`);
            }
        },
        code: (() => {
